...
// MwValidator2I18n i18n 国际化
// @param I18n
// @return middleware.Middleware
func MwValidator2I18n(I18n *i18n.I18n, lg *zap.Logger) Middleware {
	l := utils.NewWLog(lg)
	return func(handler Handler) Handler {
		return func(c *gin.Context, req any) (res any, err error) {
			if v, ok := req.(validator); ok {
				if err = v.Validate(); err != nil {
					...
					log.Printf("i18nKey:%v,params:%v\n", i18nKey, params)
					msg := I18n.Tr(en, i18nKey, params)
					// 记录错误日志
					l.WithCtx(c.Request.Context()).Info(`validate-err`,
...
func getValidateKey(str string) (string, map[string]any) {
	var msgKey string
