// Package server
// @file      : server.go
// @author    : china.gdxs@gmail.com
// @time      : 2023/11/7 17:04
// @Description: created server
package server

import (
	"fmt"
	"github.com/emicklei/proto"
	"github.com/spf13/cobra"
	"log"
	"os"
	"path"
	"strings"
)

// CmdServer the service command.
var CmdServer = &cobra.Command{
	Use:   "server",
	Short: "Generate the proto Server implementations",
	Long:  "Generate the proto Server implementations. Example: proto server api/xxx.proto -t=internal/service",
	Run:   run,
}
var targetDir string

func init() {
	CmdServer.Flags().StringVarP(&targetDir, "target-dir", "t", "internal/service", "generate target directory")
}

func run(cmd *cobra.Command, args []string) {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "Please specify the proto file. Example: proto server api/xxx.proto")
		return
	}
	var (
		err        error
		reader     *os.File
		definition *proto.Proto
	)
	if reader, err = os.Open(args[0]); err != nil {
		log.Fatal(err)
	}
	defer reader.Close()

	parser := proto.NewParser(reader)
	if definition, err = parser.Parse(); err != nil {
		log.Fatal(err)
	}

	var (
		pkg, pkgName string
		res          []*Service
	)
	proto.Walk(definition,
		proto.WithOption(func(o *proto.Option) {
			if o.Name == "go_package" {
				pkgs := strings.Split(o.Constant.Source, ";")
				pkg = pkgs[0]
				pkgName = pkgs[1]
			}
		}),
		proto.WithService(func(s *proto.Service) {
			cs := &Service{
				Package:     pkg,
				PackageName: pkgName,
				Service:     s.Name,
			}
			for _, e := range s.Elements {
				r, ok := e.(*proto.RPC)
				if ok {
					cs.Methods = append(cs.Methods, &Method{
						Service: s.Name, Name: r.Name, Request: r.RequestType,
						Reply: r.ReturnsType, Type: getMethodType(r.StreamsRequest, r.StreamsReturns),
					})
				}
			}
			res = append(res, cs)
		}),
	)
	if _, err = os.Stat(targetDir); os.IsNotExist(err) {
		fmt.Printf("Target directory: %s does not exsit\n", targetDir)
		return
	}
	for _, s := range res {
		to := path.Join(targetDir, strings.ToLower(s.Service)+".go")
		if _, err = os.Stat(to); !os.IsNotExist(err) {
			fmt.Fprintf(os.Stderr, "%s already exists: %s\n", s.Service, to)
			continue
		}
		var b []byte
		if b, err = s.execute(); err != nil {
			log.Fatal(err)
		}
		if err = os.WriteFile(to, b, 0o644); err != nil {
			log.Fatal(err)
		}
		fmt.Println(to)
	}
}

func getMethodType(streamsRequest, streamsReturns bool) MethodType {
	if !streamsRequest && !streamsReturns {
		return unaryType
	} else if streamsRequest && streamsReturns {
		return twoWayStreamsType
	} else if streamsRequest {
		return requestStreamsType
	} else if streamsReturns {
		return returnsStreamsType
	}
	return unaryType
}
